    ...
    <v-stepper v-model="step">
      ...
      <v-stepper-items>
        ...
        <v-stepper-content step="2">
          <div>
            <v-file-input truncate-length="26" accept="video/*" @change="handleFile"></v-file-input>
          </div>
        </v-stepper-content>
...
export default {
  ...
  computed: {
    ...mapState('videos-upload', ["videoUploadPromise", "active", "step"]),
    uploadType() {
      return {
        ...UPLOAD_TYPE
      }
  ...
  methods: {
    ...
    async saveTrick() {
      if (!this.videoUploadPromise) {
        console.log("videoUploadTask is null");
        return
      }

      const video = await this.videoUploadPromise;
      console.log(video)
